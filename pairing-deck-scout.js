(() => {
  ...
  async function scoutRows(tid, ids = []) {
    ...
    let query = pairingClient
      ...
      .eq('tid', String(tid));
    if (ids.length) query = query.in('player_id', ids);
  ...
  async function renderScoutMatch(data, table) {
    ...
    const cards = ids.map((id, index) => {
      const display = names[id] || (index === 0 ? match.player_name : match.opponent_name) || id;
    ...
    box.querySelectorAll('[data-scout-player]').forEach(select => {
      select.addEventListener('change', async () => {
        ...
        try {
          ...
          setStatus(`已記錄：${deckName}`, 'success');
    ...
    if (!deckCatalog.length) setStatus('請先在「牌組清單」新增選項，再開始紀錄。', 'error');
  }
  ...
  async function loadTable(table) {
    ...
    try {
      ...
      setStatus(data.match ? '已載入，選擇牌組後會自動儲存。' : '找不到這一桌。', data.match ? 'success' : '');
    } catch (error) {
  ...
  async function refreshStats(tid) {
    ...
    if (!box || !tid) return;
    const rows = await scoutRows(tid);
    const counts = new Map();
    for (const row of rows) counts.set(row.deck_name, (counts.get(row.deck_name) || 0) + 1);
    ...
    box.innerHTML = `
      <div class="pairing-scout-stats-head"><strong>牌組分布</strong><span>已記錄 ${total} 位玩家</span></div>
  ...
  async function decorateMainPairing(detail) {
    ...
    const rows = await scoutRows(tid, [opponentId]).catch(() => []);
    const known = rows[0]?.deck_name || '';
    ...
    badge.className = `pairing-scout-known${known ? '' : ' unknown'}`;
    badge.innerHTML = known ? `已記錄牌組：<strong>${esc(known)}</strong>` : '牌組尚未記錄';
